...
pipeline {
    ...
    stages {
            ...
            steps {
                withSonarQubeEnv('Sonar-Server') {
                    sh '''
                        ...
                        -Dsonar.projectName=app-product-catalog-service \
        ...
        stage('Authenticate & Authorize Snyk') {
            steps {
                withCredentials([string(credentialsId: 'Snyk-API-Token', variable: 'SNYK_TOKEN')]) {
                    sh '''
                        ${SNYK_HOME}/snyk-linux auth $SNYK_TOKEN
                    '''
                }
            }
        }
        ...
        stage('OPA Dockerfile Vulnerability Scan') {
            steps {
                sh '''
                    ...
                    openpolicyagent/conftest test --policy docker-opa-security.rego Dockerfile || true
                '''
            }
        }
        ...
        stage('Snyk SCA Test | Dependencies') {
            steps {
                sh '''
                    ${SNYK_HOME}/snyk-linux test --docker misterseyiayeni/productcatalogservice:latest || true
                '''
            }
        }
